...
getPaperFromSrc = function( src, uri ) {
  ...
  // combine all of meta into paper

  // console.log('docdoc:', $(src).html())
  
  var meta = getMetadata(src, url.parse(uri).hostname, uri)
  ...
  // console.log('Half paper product', paper )
  if(!paper.doi)
...
module.exports = 
  function (ctx, req, res) {
    ...
    if(req.method == 'GET') {
      ...
      console.log("POST", ctx, req, res)

      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.write("POST", ctx, req, res)
      res.end()
    }
  };
...
function getSchema(nodes, queries) {
  ...
  */
  ...
  try {
    ...
    $(nodes).find('[type="application/ld+json"]').each(function() {
    ...
    // if (src) {
      ...
      for (var q of queries) {
        ...
        if(src[q]) { // if this exist, dig deeper {
  ...
  // if schema.org doesn't exist, we check for parsely
...
/*
  ...
  try {
    // $($('[type="application/ld+json"]')[0]).text()
    src = $($(nodes).find('[name="parsely-page"]')[0]).text()
    console.log(src)
    if (src) {
      obj = JSON.parse(src)
      if(obj[queries[0]])
        return obj[queries[0]]
    }
    return src
  } catch(err) {
    return undefined
...
function getMetadata(data, origin, uri) {

  // var doc = document.createElement( 'html' )
  // doc.innerHTML = data

  var doc = jsdom(data, {features: {
    FetchExternalResources: false,
    ProcessExternalResources: false
  }});

  // the metadataparser is incredibly incomplete
  // console.log("doc:", doc);

  // we're not only grabbing info from the head tag, but also scouring the body
  // sometimes for author names, etc.
  ...
  */
  ...
  // console.log('url:', window.location.search.substring(window.location.search.indexOf("url=") + 4),  'head:', $(doc).find('meta, link, title').each(function() {
  //   console.log(this)
  // }))
  // const URL = require('url');
  // var parsedUrl = new URL(window.location.href);
  // var _url = parsedUrl.searchParams.get("url")
  // var _title = parsedUrl.searchParams.get("title")
  ...
  // const title = metaParse({property: "property", [
  //       ['[property="og:title"]', node => $(node).attr('content')],
  //       ['title', node => $(node).text()]
  //     ], doc})
  ...
  //   ]
  // };
  // console.log('description rules:',metadataparser.metadataRuleSets.description)
  // const customDescriptionRuleSet = metadataparser.metadataRuleSets.description;
  // customDescriptionRuleSet.rules.push(
  //   ['meta[property="twitter:description"]', t => t.getAttribute('content')]
  // );
  // pageDescription = metadataparser.getMetadata(doc, url, {description: customDescriptionRuleSet});

  // var meta = metadataparser.getMetadata(doc, url);
  // console.log("meta:", meta, "pagedesc:", pageDescription);
